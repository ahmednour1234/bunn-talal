<div>
    ...
    <form wire:submit="save">
        <div class="bg-card rounded-2xl shadow-sm border border-primary-100 p-6 mb-4">
            <h2 class="text-lg font-semibold text-primary-700 mb-4 pb-3 border-b border-gray-100">
                بيانات الإرجاع
            </h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input
                        type="text"
                        wire:model.live.debounce.500ms="orderSearch"
                        placeholder="اكتب رقم الفاتورة أو اسم العميل..."
                        class="w-full mb-3 px-4 py-3 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary-300"
                    >

                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        طلب المبيعات <span class="text-red-500">*</span>
                    </label>

                    <select
                        wire:model.live="sale_order_id"
                        class="w-full px-4 py-3 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary-300 @error('sale_order_id') border-red-400 @enderror"
                    >
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        تاريخ الإرجاع <span class="text-red-500">*</span>
                    </label>
        ...
        @if(!empty($items))
                                        ...
                                        @if(!empty($item['available_units']))
                                            ...
                                            >
                                                @foreach($item['available_units'] as $unitOpt)
                                                    <option value="{{ $unitOpt['id'] }}">
                                                        {{ $unitOpt['name'] }} ({{ $unitOpt['symbol'] }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        @else
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        @if($item['max_quantity'] > 0)
                                            <p class="text-[11px] text-gray-400 mt-1">
                                                أقصى: {{ $item['max_quantity'] }}
                                            </p>
                                        @endif
                                    </td>

                                    <td class="px-4 py-3">
                                        <div class="px-3 py-2.5 bg-gray-50 rounded-lg text-sm text-gray-700">
            ...
            <div class="flex items-center gap-3">
                <button
                    type="submit"
                    class="inline-flex items-center gap-2 px-6 py-3 bg-primary-600 text-white rounded-xl hover:bg-primary-700 transition-colors font-medium shadow-sm"
                >
                ...
                <a
                    ...
                    class="inline-flex items-center gap-2 px-6 py-3 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition-colors font-medium"
                >
                    إلغاء
                </a>
            </div>
