<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-primary-700">إدارة الحسابات</h1>
            <p class="text-sm text-gray-500 mt-1">عرض وإدارة جميع الحسابات</p>
        </div>
        <div class="flex items-center gap-2">
            <x-button variant="secondary" size="sm" href="{{ route('accounts.export.excel') }}">
            ...
            <x-button variant="secondary" size="sm" href="{{ route('accounts.export.pdf') }}">
            ...
            @if(auth('admin')->user()?->hasPermission('accounts.create'))
                <x-button variant="primary" href="{{ route('accounts.create') }}">
    ...
    <div class="bg-card rounded-2xl shadow-sm border border-primary-100 p-4 mb-6">
        <div class="relative">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="بحث بالاسم أو رقم الحساب..."
    ...
    {{-- Table --}}
    <x-data-table :headers="['#', 'اسم الحساب', 'رقم الحساب', 'يظهر للمندوب', 'الحالة', 'الإجراءات']">
        @forelse($accounts as $account)
            <tr class="hover:bg-primary-50/50 transition-colors">
                <td class="px-6 py-4 text-gray-500">{{ $account->id }}</td>
                <td class="px-6 py-4 font-medium text-gray-800">{{ $account->name }}</td>
                <td class="px-6 py-4 text-gray-600 text-sm font-mono" dir="ltr">{{ $account->account_number }}</td>
                <td class="px-6 py-4">
                    @if($account->visible_to_delegate)
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">نعم</span>
                    @else
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-500">لا</span>
                    @endif
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                        @if(auth('admin')->user()?->hasPermission('accounts.edit'))
                            <button wire:click="toggleActive({{ $account->id }})"
                                class="p-2 {{ $account->is_active ? 'text-orange-600 hover:bg-orange-50' : 'text-green-600 hover:bg-green-50' }} rounded-lg transition-colors"
                            ...
                            <a href="{{ route('accounts.edit', $account) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="تعديل">
                                <x-icon name="pencil" class="w-4 h-4" />
                            </a>
                        @endif
                        @if(auth('admin')->user()?->hasPermission('accounts.delete'))
                            <button wire:click="delete({{ $account->id }})" wire:confirm="هل أنت متأكد من حذف هذا الحساب؟" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="حذف">
        ...
        @empty
            <tr>
                <td colspan="6" class="px-6 py-12 text-center text-gray-400">
                    <x-icon name="banknotes" class="w-12 h-12 mx-auto mb-3 text-gray-300" />
                    <p>لا يوجد حسابات مسجلة</p>
                </td>
            </tr>
        @endforelse
