                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-primary-50/60">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-right font-semibold text-primary-700 w-40">وحدة الصرف</th>
                                <th class="px-4 py-3 text-right font-semibold text-primary-700 w-36">الكمية</th>
                                <th class="px-4 py-3 text-right font-semibold text-primary-700 w-32">سعر التكلفة</th>
                                <th class="px-4 py-3 text-right font-semibold text-primary-700 w-32">سعر البيع</th>
                                <th class="px-4 py-3 text-center font-semibold text-primary-700 w-14">حذف</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                            @foreach($items as $index => $item)
                                <tr wire:key="dispatch-item-{{ $index }}" class="align-top">
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        <input type="number" wire:model="items.{{ $index }}.cost_price" step="0.01" min="0"
                                            class="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-sm">
                                        @error("items.{$index}.cost_price")<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                                    </td>
                                    <td class="px-4 py-3">
                                        <input type="number" wire:model="items.{{ $index }}.selling_price" step="0.01" min="0"
                                            class="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-sm">
                                        @error("items.{$index}.selling_price")<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                                    </td>
