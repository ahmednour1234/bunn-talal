<x-layouts.app>
    ...
    {{-- Search / Filter Form --}}
    <form method="GET" action="{{ route('customers.index') }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5">
        <div class="flex flex-wrap gap-3">
            ...
            <select name="classification" class="border border-gray-200 rounded-xl bg-gray-50 text-sm px-3 py-2.5 focus:bg-white focus:ring-2 focus:ring-primary-300 transition-all">
                <option value="">كل التصنيفات</option>
                <option value="premium" @selected($classification === 'premium')>مميز</option>
                <option value="medium"  @selected($classification === 'medium')>متوسط</option>
                <option value="regular" @selected($classification === 'regular')>عادي</option>
            </select>
            <select name="area_id" class="border border-gray-200 rounded-xl bg-gray-50 text-sm px-3 py-2.5 focus:bg-white focus:ring-2 focus:ring-primary-300 transition-all">
                <option value="">كل المناطق</option>
                @foreach($areas as $area)
                    <option value="{{ $area->id }}" @selected((string)$areaId === (string)$area->id)>{{ $area->name }}</option>
                @endforeach
            </select>
            <button type="submit" class="px-4 py-2.5 bg-primary-700 text-white text-sm font-medium rounded-xl hover:bg-primary-800 transition-colors">
                بحث
            </button>
            @if($search || $classification || $areaId)
                <a href="{{ route('customers.index') }}" class="px-4 py-2.5 bg-gray-100 text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-200 transition-colors">
                    مسح
                </a>
            @endif
        </div>
            ...
            <table class="w-full text-sm text-right">
                ...
                <tbody>
                    ...
                    <tr class="{{ $i % 2 === 0 ? 'bg-white' : 'bg-stone-50/40' }} hover:bg-stone-50 transition-colors border-b border-gray-50">
                        <td class="px-4 py-3 text-xs text-gray-400 font-mono text-center">{{ $customers->firstItem() + $i }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-gray-600 text-sm">{{ $customer->area?->name ?? '—' }}</td>
                        <td class="px-4 py-3">
                            ...
                            <div class="flex items-center gap-1.5 justify-center">
                                @if(auth('admin')->user()?->hasPermission('customers.view'))
                                    <a href="{{ route('customers.show', $customer) }}"
                                        class="w-8 h-8 rounded-lg bg-primary-50 text-primary-700 hover:bg-primary-100 flex items-center justify-center transition-colors" title="عرض الملف">
                                ...
                                @if(auth('admin')->user()?->hasPermission('customers.edit'))
                                    <form method="POST" action="{{ route('customers.toggle-active', $customer) }}">
                                        @csrf
                                        <button type="submit"
                                            class="w-8 h-8 rounded-lg bg-stone-100 text-stone-600 hover:bg-stone-200 flex items-center justify-center transition-colors"
                                    ...
                                    <a href="{{ route('customers.edit', $customer) }}"
                                        class="w-8 h-8 rounded-lg bg-stone-100 text-stone-600 hover:bg-stone-200 flex items-center justify-center transition-colors" title="تعديل">
                                ...
                                @if(auth('admin')->user()?->hasPermission('customers.delete'))
                                    <form method="POST" action="{{ route('customers.destroy', $customer) }}"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="w-8 h-8 rounded-lg bg-stone-100 text-red-600 hover:bg-red-50 flex items-center justify-center transition-colors" title="حذف">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                                        </button>
                                    </form>
                                @endif
