...
class TripShow extends Component
{
    ...
    public function approveSettlement(): void
    {
        ...
        DB::transaction(function () use ($admin) {
            ...
            if ($trip->settlement_treasury_id && $trip->settlement_cash_actual > 0) {
                $alreadyDeposited = \App\Models\TreasuryTransaction::where('reference_number', (string) $trip->id)
                    ->where('type', 'deposit')
                    ->where('treasury_id', $trip->settlement_treasury_id)
                    ->exists();
    ...
    public function reopenSettlement(): void
    {
        ...
        DB::transaction(function () {
            ...
            if ($wasApproved) {
                ...
                // ── 1. Reverse stock from live trip data ─────────────────
                // dispatched - sold (non-cancelled) + returned = what delegate had
                $rows = [];

                foreach ($this->trip->dispatches()->with('items')->get() as $dispatch) {
                    foreach ($dispatch->items as $item) {
                        $rows[$item->product_id] = ($rows[$item->product_id] ?? 0.0) + (float) $item->quantity;
                    }
                }
                ...
                // ── 2. Reverse cash: find and delete the treasury deposit ─
                if ($this->trip->settlement_treasury_id && $this->trip->settlement_cash_actual > 0) {
                    $tx = \App\Models\TreasuryTransaction::where('treasury_id', $this->trip->settlement_treasury_id)
                        ->where('type', 'deposit')
                        ->where('reference_number', (string) $this->trip->id)
                        ->latest()
                        ->first();
